...
class FelzenszwalbSuperpixel:
    ...
    def colorize_segments(self, segments):
        """Create a colored visualization of segments with distinct colors"""
        # Get unique segment IDs
        unique_segments = np.unique(segments)
        num_segments = len(unique_segments)
        
        # Create a color mapping that ensures distinct colors
        if num_segments <= 4:
            # For small number of segments, use predefined distinct colors
            predefined_colors = [
                [1.0, 0.0, 0.0],  # Red
                [0.0, 1.0, 0.0],  # Green  
                [0.0, 0.0, 1.0],  # Blue
            ...
            colors = predefined_colors[:num_segments]
        else:
            # For larger number of segments, use HSV color space for better distribution
            hues = np.linspace(0, 1, num_segments, endpoint=False)
            colors = []
            for hue in hues:
                # Use matplotlib's hsv_to_rgb for color conversion
                from matplotlib.colors import hsv_to_rgb
                rgb = hsv_to_rgb([hue, 0.8, 0.9])  # High saturation and brightness
                colors.append(rgb)
    ...
    def visualize_final_result(self):
        ...
        unique_segs = np.unique(self.current_segments)
        axes[1].set_title(f'Superpixel Segmentation\n({len(unique_segs)} segments)')
        axes[1].axis('off')
        
        # Segment boundaries overlay
        boundaries = self.get_segment_boundaries(self.current_segments)
        axes[2].imshow(self.original_image, cmap='gray')
        axes[2].imshow(boundaries, alpha=0.3, cmap='Reds')
        axes[2].set_title('Segment Boundaries')
        axes[2].axis('off')
        ...
        print(f"\nSegment IDs found: {sorted(unique_segs)}")
        for seg_id in unique_segs:
            count = np.sum(self.current_segments == seg_id)
            print(f"Segment {seg_id}: {count} pixels")
    
    def get_segment_boundaries(self, segments):
        ...
        for y in range(1, self.height-1):
            for x in range(1, self.width-1):
                center = segments[y, x]
                # Check if any neighbor has different segment ID
                neighbors = [
                    segments[y-1, x], segments[y+1, x],
                    segments[y, x-1], segments[y, x+1]
                ]
                if any(neighbor != center for neighbor in neighbors):
                    boundaries[y, x] = True
        
        return boundaries

def create_sample_image():
    """Create a sample image for demonstration with 4 distinct foreground regions"""
    # Create a larger image with clear distinct regions
    image = np.zeros((120, 120), dtype=np.float32)
    
    # Add 4 distinct foreground regions with very different intensities
    # Make the intensity differences larger
    image[20:50, 20:50] = 60    # Dark gray square (top-left)
    image[20:50, 70:100] = 120  # Medium gray square (top-right) 
    image[70:100, 20:50] = 180  # Light gray square (bottom-left)
    image[70:100, 70:100] = 240 # Very light gray square (bottom-right)
    
    # Add very minimal noise to preserve distinct regions
    ...
    # Add some spacing between regions to help segmentation
    # Create clear boundaries
    image[0:20, :] = 0      # Top border
    image[50:70, :] = 0     # Horizontal separator
    image[:, 0:20] = 0      # Left border
    image[:, 50:70] = 0     # Vertical separator
    image[100:120, :] = 0   # Bottom border
    image[:, 100:120] = 0   # Right border
...
def main():
    ...
    segmenter = FelzenszwalbSuperpixel(
        image=image,
        scale=20,      # Much lower scale = more segments
        sigma=0.3,     # Less smoothing to preserve boundaries
        min_size=200   # Larger minimum segment size to get 4 main regions
    )
